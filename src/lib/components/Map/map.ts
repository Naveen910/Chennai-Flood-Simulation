...
const MAP_CONSTANTS = {
  ...
  CONFIG: {
    center: [80.15, 13.1] as [number, number],
    maxZoom: 15,
    hash: true,
    minZoom: 12,
    pitch: 45,
...
const initializePMTiles = () => {
  ...
  const p = new PMTiles('basemap.pmtiles');
  protocol.add(p);
  return p;
};
    ...
    this.map.addLayer(
      {
        ...
        'source-layer': 'stream_influence_reclass',
        ...
        navigator.geolocation.getCurrentPosition(
          position => {
            ...
            if (
              ...
              resolve({
                type: 'error',
                message: `We couldn't locate you, you seem to be outside the map bounds of Chennai. Bet the traffic is better.`
              });
            }
          },
          error => {
